...
/**
 * Multi-line time-series plot of frontier size per player per turn.
 * Data: legalMovesHistory from gameStore, or overrideHistory when provided.
 */
export const FrontierSizePlot: React.FC<{
    ...
    const { lines, maxTurn, maxCount, width, height, pad, plotW, plotH } = useMemo(() => {
        ...
        for (const e of legalMovesHistory) {
            for (const m of Object.values(e.byPlayer)) {
                const v = typeof m === 'number' ? 0 : m.frontierSize ?? 0;
        ...
        const lines = PLAYER_ORDER.map((player) => {
            ...
            legalMovesHistory.forEach((e: LegalMovesHistoryEntry, turn) => {
                ...
                const v = m !== undefined ? (typeof m === 'number' ? undefined : m.frontierSize) : undefined;
                if (v !== undefined) {
    ...
    // Ensure maxCount is rounded/appropriate for ticks
    const displayMax = Math.ceil(maxCount);
        ...
        <div className="px-3 py-3 border-t border-charcoal-700">
            ...
            <svg width={width} height={height} className="overflow-visible">
                ...
                {[0, displayMax].map((tickVal) => {
                    if (tickVal > maxCount && tickVal !== displayMax) return null;
                    const y = pad.top + plotH - (tickVal / Math.max(0.1, maxCount)) * plotH;
                    return (
                        <g key={`ytick-${tickVal}`}>
