...
/**
 * Multi-line time-series plot of Territory Control % per player per turn.
 */
export const TerritoryControlPlot: React.FC<{
    ...
    const { lines, maxTurn, width, height, pad, plotW, plotH, toY } = useMemo(() => {
        const w = 740;
        const h = 180;
        ...
        if (legalMovesHistory.length === 0) {
            return { lines: [], maxTurn: 1, width: w, height: h, pad, plotW: w - pad.left - pad.right, plotH: h - pad.top - pad.bottom, toY: toY_default };
        }

        const maxTurn = legalMovesHistory.length - 1;

        // Fixed range 0 to 1 for percentage
        const minCount = 0.0;
        const range = 1.0;
        ...
        const lines = PLAYER_ORDER.map((player) => {
            ...
            legalMovesHistory.forEach((e: LegalMovesHistoryEntry, turn) => {
                const v = e.advanced?.[player]?.territory_ratio;
                if (v !== undefined) {
        ...
        return { lines, maxTurn: Math.max(1, maxTurn), width: w, height: h, pad, plotW, plotH, toY };
