...
export const CenterProximityPlot: React.FC<{
    ...
    const { lines, maxTurn, maxCount, width, height, pad, plotW, plotH, toY } = useMemo(() => {
        ...
        for (const e of legalMovesHistory) {
            for (const p of PLAYER_ORDER) {
                const v = e.advanced?.[p]?.center_proximity;
                if (v !== undefined) {
        ...
        // Reverse Y axis so 0 (best proximity) is at the TOP.
        const toY = (count: number) => pad.top + (count / Math.max(0.1, maxCount)) * plotH;
        ...
        const lines = PLAYER_ORDER.map((player) => {
            ...
            legalMovesHistory.forEach((e: LegalMovesHistoryEntry, turn) => {
                const v = e.advanced?.[player]?.center_proximity;
                if (v !== undefined) {
    ...
    return (
        <div className="px-3 py-3 border-t border-charcoal-700">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">
                Center Proximity (Lower is Better)
            </h3>
